...
set fileLua2Luac=lua2luac.bat

rem ----- checkout the sub batch script existence -----
set fileLost=cygwin1.dll
if not exist %PATH_BUILD%\%fileLost% copy %PATH_AUTOMATIC_BUILD%\%DIR_SUB_FILES%\%fileLost% %PATH_BUILD%\%fileLost%
set fileLost=cygz.dll
if not exist %PATH_BUILD%\%fileLost% copy %PATH_AUTOMATIC_BUILD%\%DIR_SUB_FILES%\%fileLost% %PATH_BUILD%\%fileLost%
set fileLost=%fileLua2Luac%
if not exist %PATH_BUILD%\%fileLost% copy %PATH_AUTOMATIC_BUILD%\%DIR_SUB_FILES%\%fileLost% %PATH_BUILD%\%fileLost%
set fileLost=%fileSkipPngs%
if not exist %PATH_PROJ_ANDROID_STUDIO%\%fileLost% copy %PATH_AUTOMATIC_BUILD%\%DIR_SUB_FILES%\%fileLost% %PATH_PROJ_ANDROID_STUDIO%\%fileLost%
...
for /r %%i in (*.csv) do (
  ...
  if not defined bTheSameExists (
    ...
    if defined mDebugEcho echo confuse csv: %%~nxi
    call %PATH_BUILD%\%fileZipConfuse% %SUFFIX_ARCH%%SUFFIX_EXE% %%i %mConfuseLength%
  )
)
if defined mDebugEcho for /r %%i in (*.csv) do (
  set bTheSameExists=
  for /r %%j in (.\build\*) do (
    if %%i == %%j set bTheSameExists=true
  )
...
for /r %%i in (*.xml) do (
  ...
  if not defined bTheSameExists (
    ...
    set /a confuseCount+=1
    if defined mDebugEcho echo confuse xml: %%~nxi
    call %PATH_BUILD%\%fileAssetsConfuse% %SUFFIX_ARCH%%SUFFIX_EXE% %%i %mConfuseLength%
  )
)
if defined mDebugEcho for /r %%i in (*.xml) do (
  set bTheSameExists=
  if exist %mPathProjAndroidAssetsConfusion%\lint.xml if "%%~nxi" == "lint.xml" set bTheSameExists=true
  if exist %mPathProjAndroidAssetsConfusion%\AndroidManifest.xml if "%%~nxi" == "AndroidManifest.xml" set bTheSameExists=true
  for /r %%j in (.\res\*.xml) do (
...
for /r %%i in (*.lua) do (
  ...
  if not defined bTheSameExists (
    ...
    set /a confuseCount+=1
    if defined mDebugEcho echo confuse lua: %%~nxi
    call %PATH_BUILD%\%fileZipConfuse% %SUFFIX_ARCH%%SUFFIX_EXE% %%i %mConfuseLength%
  )
)
if defined mDebugEcho for /r %%i in (*.lua) do (
  set bTheSameExists=
  for /r %%j in (.\build\*) do (
    if %%~nxi == %%j set bTheSameExists=true
  )
...
for /r %%i in (*.png) do (
  ...
  if not defined bTheSameExists (
    ...
    if defined mDebugEcho echo confuse with quanting !confuseCount! png: %%~pnxi
  ) else ( 
...
rem if defined callPngquant1Deletion call %PATH_AUTOMATIC_BUILD%\automatic_build_pngquant_1_deleting_png.bat
...
for /r %%i in (*-fs8.png) do (
  ...
  if not defined bTheSameExists (
    ...
    set nameOfPng=%%~ni
    if defined mDebugEcho echo confuse with renaming -fs8.png file %%~nxi, !nameOfPng!
...
for /r %%i in (*-or8.png) do (
  ...
  if not defined bTheSameExists (
    set /a renameCountOfPngFiles+=1
    set nameOfPng=%%~ni
    if defined mDebugEcho echo confuse with renaming -or8.png file %%~nxi, !nameOfPng!
    del /q "%%~dpi\!nameOfPng:~0,-4!.png"
    rename "%%~i" "!nameOfPng:~0,-4!.png"
  )
)
...
for /r %%i in (*.ogg) do (
  ...
  if not defined bTheSameExists (
    set /a confuseCount+=1
    if defined mDebugEcho echo confuse ogg: %%~nxi
    call %PATH_BUILD%\%fileZipConfuse% %SUFFIX_ARCH%%SUFFIX_EXE% %%i %mConfuseLength%
  )
)
if defined mDebugEcho for /r %%i in (*.ogg) do (
  set bTheSameExists=
  for /r %%j in (.\build\*) do (
    if %%i == %%j set bTheSameExists=true
  )
...
for /r %%i in (*.omod) do (
  ...
  if not defined bTheSameExists (
    set /a confuseCount+=1
    if defined mDebugEcho echo confuse omod: %%~nxi
    call %PATH_BUILD%\%fileZipConfuse% %SUFFIX_ARCH%%SUFFIX_EXE% %%i %mConfuseLength%
  )
)
if defined mDebugEcho for /r %%i in (*.omod) do (
  set bTheSameExists=
  for /r %%j in (.\build\*) do (
    if %%i == %%j set bTheSameExists=true
  )
...
for /r %%i in (*.ktx) do (
  ...
  if not defined bTheSameExists (
    set /a confuseCount+=1
    if defined mDebugEcho echo confuse ktx: %%~nxi
    call %PATH_BUILD%\%fileZipConfuse% %SUFFIX_ARCH%%SUFFIX_EXE% %%i %mConfuseLength%
  )
)
if defined mDebugEcho for /r %%i in (*.ktx) do (
  set bTheSameExists=
  for /r %%j in (.\build\*) do (
    if %%i == %%j set bTheSameExists=true
  )
...
for /r %%i in (*.pvr) do (
  ...
  if not defined bTheSameExists (
    set /a confuseCount+=1
    if defined mDebugEcho echo confuse pvr: %%~nxi
    call %PATH_BUILD%\%fileZipConfuse% %SUFFIX_ARCH%%SUFFIX_EXE% %%i %mConfuseLength%
  )
)
if defined mDebugEcho for /r %%i in (*.pvr) do (
  set bTheSameExists=
  for /r %%j in (.\build\*) do (
    if %%i == %%j set bTheSameExists=true
  )
...
rd /q /s egame
rd /q /s unipaysdk_res

set aDirsUnderEntity=100001 100007 100009 100014 140001 140002 140003 140004 140005 140006 140007 140008 140009 140010 140011 140012 140013 140015 140016 140017 140018 140019 140020 140021 140022 140023 140024 140025 140026 140027
set aUnusedFiles=body.omod body.ktx js.ktx face.ktx male.ktx male1.ktx male1_alpha.ktx male1_s.ktx  male_alpha.ktx male_s.ktx female.ktx female.pvr female_alpha.ktx ying.ktx deng.ktx face.ktx face_100100.ktx face_100108.ktx face_100130.ktx face_100155.ktx lanqiu.ktx ditu.ktx ditu_alpha.ktx hulu.ktx cat.ktx baozi.ktx yusan.ktx wuqi.ktx malegai.ktx malegai_alpha.ktx malegai01.ktx malegai01_alpha.ktx 
for %%i in (%aDirsUnderEntity%) do (
  if exist entity\%%i for %%j in (%aUnusedFiles%) do (
    if exist entity\%%i\%%j del /q entity\%%i\%%j
  )
)
del /q entity\140014\body.omod
del /q entity\140014\face_100100.ktx
...
copy %PATH_BIN%\res\entity\110058\yanse.png entity\110058\yanse.png
...
REM copy %PATH_BIN%\res\entity\140014\face_100108.ktx entity\140014\face_100108.ktx
REM copy %PATH_BIN%\res\entity\140014\face_100130.ktx entity\140014\face_100130.ktx
REM copy %PATH_BIN%\res\entity\140014\male.ktx entity\140014\male.ktx
