...
static const char *sClientVersion = "0.27.3";
...
static const char *sClientVersion = "0.26.7";
...
ClientManager::ClientManager()
	...
	m_ModEditorManager(NULL), m_ModFileManager(NULL), m_ModManager(NULL), m_LoadingGame(NULL), m_MinimapRenderer(NULL), m_pTraceRoute(NULL), m_httpFileUpDownMgr(NULL)
#if OGRE_PLATFORM != OGRE_PLATFORM_LINUXPC
	,m_GVoiceMgr(NULL) 
#endif
{
...
bool ClientManager::onInitialize(const char *pkgsource, const char *datadir, int imsi, void *hinst, const char *cmdstr)
{
...
static const char *sClientVersion = "0.26.10";
...
void ClientManager::updateLoadingGame()
{
	...
	else if(loadstatus < 0)
	{
		...
		removeGame(m_LoadingGame);
		m_LoadingGame = NULL;
		if(GameEventQue::getSingletonPtr()) 
			GameEventQue::getSingletonPtr()->postLoadProgress(1000, -1);
...
{
	//SoundSystem::getSingleton().playSound2D(path, fVolume);
	if(Ogre::SoundSystem::getSingletonPtr() != NULL){
		Ogre::SoundSystem::getSingletonPtr()->playSound2D(path, fVolume);
	}
}
...
ISound* ClientManager::playSound2DControl(const char *path, float fVolume, bool loop)
{
	//return SoundSystem::getSingleton().playSound2DControl(path, fVolume, loop);
	if(Ogre::SoundSystem::getSingletonPtr() != NULL){
		return	Ogre::SoundSystem::getSingletonPtr()->playSound2DControl(path, fVolume, loop);
	}
	return NULL;
}
...
std::string ClientManager::getDeviceToken()
{
    LOG_INFO("kekeke ClientManager::getDeviceToken");
		...
		if (lang_ > 0) {
			...
			if (!nodeNotice.isNull() && !nodeCotent.isNull())
			{
				...
				if(!nodeNotice.isNull()){
					int noticeCode = atoi(nodeNotice.getText());
					if (noticeCode >= 1 && !nodeCotent.isNull() )
					{
						const char *content = nodeCotent.getText();
						showUpdateFrame(content, 2, noticeCode, 0);
					}
...
void ClientManager::setDownloadStop(bool stop)
{
	Ogre::DownloadManager::getSingleton().setDownloadStop(stop);
}

void ClientManager::UiBreakAllLoad()
{
	Ogre::DownloadManager::getSingleton().UiBreakAllLoad();
}
...
void reportStatisticsJson(const char* json)
{
#if OGRE_PLATFORM == OGRE_PLATFORM_WIN32
	int platform = 3;
#elif OGRE_PLATFORM == OGRE_PLATFORM_ANDROID || OGRE_PLATFORM == OGRE_PLATFORM_LINUXPC
	...
	jsonobj->import("ieventTime", jsonxx::Number(GetTimeStamp()));
	jsonobj->import("ioptype", jsonxx::Number(10000));
	g_ClientMgr.postStatisticsData(jsonobj->json());
	delete jsonobj;
}
...
bool ClientManager::initGameData()
{
	...
	//ogre也有需要上报的
    Ogre::SetStatisticsJson(&reportStatisticsJson);
	return true;
}
...
//用于设备推送, 推送需要知道设备是苹果还是华为,小米等
std::string ClientManager::getDeviceVender()
{
#if OGRE_PLATFORM == OGRE_PLATFORM_APPLE
    return "apple";
#else
