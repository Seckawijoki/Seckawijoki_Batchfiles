	...
	const char *datadir, int imsi, void *hinst, const char *cmdstr)
{
	...
	//create���������ClientManagerPC��ClientManagerMobile��ʵ��
...
bool ClientManager::onResetRender(int width, int height)
{
	...
	if(height !=0 && width/height >= 1.87f){  //android��ȫ��������֮ǰ�����1.86
	...
	if(height !=0 && width/height >= 2.f){  //android��ȫ��������֮ǰ�����1.86
...
{
	//if(getApiId() >= 300 && getApiId() != 999)	//��������η���
	//	return false;
...
{
	//if(getApiId() >= 300 && getApiId() != 999)	//���κ����Gvoice
	//	return false;
...
bool ClientManager::showImagePicker(std::string path, int type, bool crop/* =true */, int x/* =1280 */, int y/* =1280 */)
{
	LOG_INFO("ClientManager::showImagePicker");
		 ...
		 if(isopen)
		  return false;
		 isopen = true;
		 TCHAR   szFilename[MAX_PATH]   =   TEXT("����ͼ��.png");
		 BOOL   bResult   =   FALSE;   
		 ...
		 /// �ڱ��洰�� ������������ ��������ʾ��
		 ofn.lpstrFilter = TEXT( "(ͼ���ļ�)*.*\0*.png;*.jpg\0\0";); 		  
		 ///�ļ�����szFilename���棻
...
bool ClientManager::initEngine()
{
	...
	m_windowModeStyle = GetWindowLong((HWND)m_hWnd, GWL_STYLE);//��ô��ڷ��   
...
void ClientManager::setFullscreen(bool b)
{
	...
	if (m_isFullScreen)
	{
		...
		style = style & (~WS_CAPTION) & ~(WS_BORDER) & ~WS_THICKFRAME; //����ȫ����ʾ�Ҳ��ɸı��С  
	...
	else
	{
		...
		int screenW = GetSystemMetrics(SM_CXSCREEN);
		int screenH = GetSystemMetrics(SM_CYSCREEN);
		SetWindowLong(hWnd, GWL_STYLE, m_windowModeStyle); //���ô��ڷ��   
		////////LONG style = GetWindowLong(hWnd, GWL_STYLE);//��ô��ڷ��    
...
{
	//��ȡmaindata�ļ�,�ȷ�һ���汾maindata����װ����,���ٺ����汾cdn������
#ifndef _WIN32
	do
	{
...
void ClientManager::initGameDataStep(LoadStepCounter &loadcounter)
{
	...
	if ( getGameData( "debug_ui") == 1 ) {
		//�������κν���XML��LUA����F12ȥ�󷢼��أ���F11��ʾ�����Ե���UIʹ��
	...
	else if(loadcounter.getStage() == 1) //�ֲ����ص���toc�����õ�xml��lua�ļ�
	{
...
bool ClientManager::initGameData()
{
	...
	struct timeval stNow;
    gettimeofday(&stNow, NULL);
    //TODO ���Դ�serverdata.list�����ȡ�����������http url���� __init__����
	#ifndef IWORLD_SERVER_BUILD
...
{
	String search = FileManagerWeb::getSingleton().GeneralRealName(path);
	int nHaveLoaded =  Ogre::DownloadManager::getSingleton().FileLoadedSize(search);	//�����صĴ�С
	int nToLoadSize = FileManagerWeb::getSingleton().getFileSize(path);					//��Ҫ���صĴ�С
...
bool ClientManager::setupRenderer()
{
	...
	if ( lang_ == LANGUAGE_EN || lang_ > 2 )
	{
		...
		RFontBase::m_DrawRight2Left = (lang_==8); //����������Ҫ������������
	}
...
void ClientManager::loadGameCfgXml(Ogre::XMLData &config)
{
	...
	if (!nodeNotice.isNull() && !nodeCotent.isNull())
	{
		...
		if (lang_ > 0) {
			...
			XMLNode nodeCotent;
			if (evn_ >= 10) {
				nodeCotent = config.getRootNode().getChild("noticeContent_x");  		//����Ĭ��ʹ��_x
			}
			else {
				nodeCotent = config.getRootNode().getChild("noticeContent_1"); 			//�����ĵ�����£�Ĭ��ʹ��Ӣ��
			}
	...
	if(!briefSwitch.isNull() && atoi(briefSwitch.getText()) == 1)
	{
		...
		if(!noticeBrief.isNull())
		{
			...
			if (lang_ > 1) {
				XMLNode noticeBrief;
				if (evn_ >= 10) {
					noticeBrief = config.getRootNode().getChild("noticeBrief_x");	//��������ĵ�����£�Ĭ��ʹ��_x
				}	else {
					noticeBrief = config.getRootNode().getChild("noticeBrief_1");  	//�����ĵ�����£�Ĭ��ʹ��Ӣ��
				}
